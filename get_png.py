...
class GZHU(object):
    ...
    def sign(self,room,cfg,dev_id):
        url='http://update.unifound.net/wxnotice/s.aspx?c='+str(1)+'_Seat_'+str(dev_id)+'_1EW'
        print(cfg,"签到链接：",url)
...
def start():
    with open('config.json', 'r') as fp:
        ...
        #构建
        print(len(room_datas['data']))
        for data in room_datas['data']:
            c=data['devName']
            devSn=data['devSn']
            g.sign(cfg['room'],c,devSn)
